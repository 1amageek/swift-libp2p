// LogShim.swift
// A no-op logging shim for the libp2p node. swift-log's `Logger` pulls in
// Foundation and an `any`-typed metadata bag, neither of which is available under
// Embedded. The node's data path emits no logs; this shim gives call sites a
...
/// A no-op logger for the node's data path.
///
/// All methods are empty `@inlinable` no-ops; under `-c release` + `-wmo` the
/// optimiser removes every call. This keeps the surface free of swift-log
