// EmbeddedRawConnection.swift
// The raw, Foundation-free `[UInt8]` connection seam — the boundary a transport
// provides and over which the Embedded security / mux / negotiation layers run.
...
///
/// This is the Embedded analogue of the host `RawConnection` / `SecuredConnection`
...
///
/// Implementations are injected as a concrete generic parameter (`<R: EmbeddedRawConnection>`),
/// never as `any`, so the upper layers specialise monomorphically under Embedded.
public protocol EmbeddedRawConnection: Sendable {
    ...
    /// require more bytes treat that as ``EmbeddedNodeError/unexpectedEndOfStream``.
    ///
    /// - Throws: ``EmbeddedNodeError/connectionClosed`` if the connection is closed,
    ///   ``EmbeddedNodeError/transportFailure`` on an I/O error.
