...
**内部利用（non-exported）:**
- `P2PIdentify`（`LifecycleService` / `StreamService` / `PeerObserver` / `DiscoveryPipeline` 統合により、AutoNAT/CircuitRelay/DCUtR/NAT は component として注入）
...
`Node` は SwiftUI 的な `NodeGroupBuilder` DSL を公開し、受け取った component 群を
...
- **`LifecycleService`**: ライフサイクル管理（shutdown）。全サービス共通。
- **`StreamService`**: インバウンドストリーム処理（protocolIDs/handleInboundStream）。
- **`PeerObserver`**: ピア接続/切断の観察（peerConnected/peerDisconnected）。
- **`DiscoveryService`**: announce/find/knownPeers を提供。
- **capability consumer protocols**: `LocalIdentityConsumer`, `ListenAddressConsumer`, `SupportedProtocolsConsumer`, `StreamOpeningConsumer`
- **`StreamOpeningActivatable`**: stream opener を受け取って起動する一段の runtime role
- **`ActivatableService`**: opener などの capability が initializer 済みな service/discovery source の起動フェーズ

`ServiceComponent` / `DiscoveryComponent` は registration 時に role を確定し、
`NodeRuntime` はそれを `RuntimeServices` に正規化して扱う。起動時の runtime downcast で
"何のサービスか" を推測しない。
組み込み helper は runtime capability から service を生成できるため、
利用者が `attach...` で後注入する必要はない。
`StreamOpeningActivatable` を宣言する component では
`activatesWithStreamOpening()` が正規の registration になる。

起動時のディスパッチ:
1. **ハンドラ登録**: normalized `streamServices` の `protocolIDs` からハンドラを登録
...
ランタイムのディスパッチ:
- **接続時**: 登録済み `PeerObserver` のみに `peerConnected(peer)` を通知（最初の接続のみ、重複除外済み）
- **切断時**: 登録済み `PeerObserver` のみに `peerDisconnected(peer)` を通知（残接続なしの場合のみ）
- **シャットダウン時**: 登録済み `LifecycleService` に `await shutdown()` で停止

discovery integration も `DiscoveryPipeline` に含まれる `DiscoveryService` だけが対象になる。
...
- [x] **Discovery capability統合** - LifecycleService/StreamService/PeerObserver で統一
