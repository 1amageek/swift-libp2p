/// MockSecuredConnection - Test infrastructure for Yamux tests
import Foundation
import NIOCore
import Synchronization
@testable import P2PCore

/// A mock SecuredConnection for testing Yamux.
///
...
final class MockSecuredConnection: SecuredConnection, Sendable {
    ...
    func injectInbound(_ data: Data) {
        ...
        injectInbound(buffer)
    }

    /// Inject a ByteBuffer to be read by the connection (simulates receiving data from network).
    func injectInbound(_ buffer: ByteBuffer) {
        state.withLock { s in
